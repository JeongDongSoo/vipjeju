...
if($mode=='update') {
	for($i=1; $i<=$dc_no_max; $i++) {
		...
		$uque="update rc_dc_rate set dc_name='$dc_name', dc_color='$dc_color', dc_sort='$dc_sort'";
...
if($mode=='insert') {
	$ique="insert into rc_dc_rate values ('$dc_no','$dc_name','$dc_color','$dc_sort'";
...
<?
$tab=0;
$que="select * from rc_dc_rate order by dc_sort";
$sql=sql_query($que);
